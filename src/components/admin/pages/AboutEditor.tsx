...
export default function AboutEditor() {
  ...
  async function load() {
    const res = await get('/api/sections/about');
  ...
  async function save() {
    setSaving(true);
    const res = await put('/api/sections/about', { sections });
    if (res.ok) toast('Sección Acerca de actualizada');
  ...
  return (
    <div>
      <div style={S.header}>
        <h1 style={S.title}>Acerca de FEMUNDO</h1>
        <button onClick={save} disabled={saving} style={S.btn}>{saving ? 'Guardando...' : 'Guardar Cambios'}</button>
      </div>
      {sections.map((s, i) => (
        <div key={s.id || i} style={S.card}>
          <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem', color: '#1e3a8a' }}>Sección {i + 1}</h3>
          <div style={S.grid}>
            ...
            <F label="Ícono (Font Awesome)" value={s.icon} onChange={v => update(i, 'icon', v)} />
            <F label="URL Imagen" value={s.image_url} onChange={v => update(i, 'image_url', v)} />
          </div>
        </div>
      ))}
    </div>
